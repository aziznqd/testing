  ...
  return (
    ...
    >
      <div
        ...
        style={
          {
            "--gap": visible > 1 ? "4rem" : "2rem",
            "--cw": `calc((100vw - 2 * clamp(1.5rem,5vw,6rem) - (${visible} - 1) * var(--gap)) / ${visible} * ${visible === 1 ? 0.85 : 1})`,
          } as CSSProperties
        }
      >
        ...
        <div className="relative min-h-0 flex-1 pb-8">
          <div
            aria-hidden="true"
            className="absolute inset-x-0 top-[calc((100%-2rem)/2)] h-[2px] -translate-y-1/2 bg-foreground/40"
          />
          <motion.div
            style={{ x, paddingInline: "clamp(1.5rem,5vw,6rem)" }}
            className="flex h-full w-max items-center gap-[var(--gap)] py-4 transform-gpu"
          >
            {cardList.map((card) => (
              <div
                key={card.id}
                style={{ width: "var(--cw)" }}
                className="flex max-h-full flex-shrink-0 flex-col"
              >
                <Card {...card} />
              </div>
            ))}
...
export default function Experience() {
  ...
  return (
    <section className="relative z-20 text-foreground">
      {reduce ? <PlainSection /> : <PinnedSection />}
    </section>
  );
}